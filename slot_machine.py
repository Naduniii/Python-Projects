    ...
    while True:
        try:
            ...
            if bet > balance or bet <= 0:
                print(f'Invalid bet amount. You can bet between $1 and ${bet}')
            else:
                return bet
        except ValueError:
...
def calculate_payout(reels, bet):
    if reels[0] == reels[1] == reels[2]:
        return bet * 10
    if reels[0] == reels[1] or reels[0] or reels[2] or reels[1] == reels[2]:
        return bet * 2
    return 0
...
def main():
    ...
    print(f'You start with a balnce of ${balance}.\n')
    ...
    while balance > 0:
        ...
        play_again = input('Dp you want to play again? (y/n): ').lower()
        if play_again  != 'y':
