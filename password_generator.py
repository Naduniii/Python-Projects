...
def generate_password(length,  include_uppercase, include_numbers,include_special):
    if length < (include_uppercase + include_numbers + include_special):
        raise ValueError('Password length is too short for the specifired criteria.')
...
def main():
    ...
    include_numbers = input('Include uppercase letters? (y/n): ').lower() == 'y'
    include_special = input('Include special chracter? (y/n): ').lower() == 'y'
    try:
